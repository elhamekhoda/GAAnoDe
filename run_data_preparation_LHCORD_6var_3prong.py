...
features_3prong=pd.read_hdf("../data/events_anomalydetection_Z_XY_qqq.features.h5")
...
mj1mj2_3prongsig = np.array(features_3prong[['mj1','mj2']])
tau21_3prongsig = np.array(features_3prong[['tau2j1','tau2j2']])/(1e-5+np.array(features_3prong[['tau1j1','tau1j2']]))
tau32_3prongsig = np.array(features_3prong[['tau3j1','tau3j2']])/(1e-5+np.array(features_3prong[['tau2j1','tau2j2']]))
...
mjmin_3prongsig = mj1mj2_3prongsig[range(len(mj1mj2_3prongsig)), np.argmin(mj1mj2_3prongsig, axis=1)]
mjmax_3prongsig = mj1mj2_3prongsig[range(len(mj1mj2_3prongsig)), np.argmax(mj1mj2_3prongsig, axis=1)]
...
tau21min_3prongsig=tau21_3prongsig[range(len(mj1mj2_3prongsig)), np.argmin(mj1mj2_3prongsig, axis=1)]
tau21max_3prongsig=tau21_3prongsig[range(len(mj1mj2_3prongsig)), np.argmax(mj1mj2_3prongsig, axis=1)]
...
tau32min_3prongsig=tau32_3prongsig[range(len(mj1mj2_3prongsig)), np.argmin(mj1mj2_3prongsig, axis=1)]
tau32max_3prongsig=tau32_3prongsig[range(len(mj1mj2_3prongsig)), np.argmax(mj1mj2_3prongsig, axis=1)]
...
pjj_3prongsig = (np.array(features_3prong[['pxj1','pyj1','pzj1']])+np.array(features_3prong[['pxj2','pyj2','pzj2']]))
Ejj_3prongsig = np.sqrt(np.sum(np.array(features_3prong[['pxj1','pyj1','pzj1','mj1']])**2, axis=1))\
    +np.sqrt(np.sum(np.array(features_3prong[['pxj2','pyj2','pzj2','mj2']])**2, axis=1))
mjj_3prongsig = np.sqrt(Ejj_3prongsig**2-np.sum(pjj_3prongsig**2, axis=1))
...
dataset_3prongsig = np.dstack((mjj_3prongsig/1000, mjmin_3prongsig/1000, (mjmax_3prongsig-mjmin_3prongsig)/1000, tau21min_3prongsig, tau21max_3prongsig, tau32min_3prongsig, tau32max_3prongsig, np.full(len(mjj_3prongsig),1)))[0]
...
if args.seed!=1:
    ...
    np.random.shuffle(dataset_3prongsig)
...
# data_all = np.concatenate((dataset_bg, dataset_sig[:n_sig], dataset_3prongsig[:n_sig]))
data_all = np.concatenate((dataset_bg, dataset_3prongsig[:n_sig]))
...
innerdata_3prong_extrasig = dataset_3prongsig[n_sig:]
innerdata_3prong_extrasig = innerdata_3prong_extrasig[(innerdata_3prong_extrasig[:,0]>minmass) & (innerdata_3prong_extrasig[:,0]<maxmass)]

## splitting extra signal into train, val and test set
n_sig_test = 20000
n_extrasig_train =  (innerdata_3prong_extrasig.shape[0]-n_sig_test)//2
...
innerdata_3prong_extrasig_test = innerdata_3prong_extrasig[:n_sig_test]
innerdata_3prong_extrasig_train = innerdata_3prong_extrasig[n_sig_test:n_sig_test+n_extrasig_train]
innerdata_3prong_extrasig_val = innerdata_3prong_extrasig[n_sig_test+n_extrasig_train:]
...
innerdata_test = np.vstack((innerdata_extrabkg1_test, innerdata_3prong_extrasig_test))

np.save(os.path.join(args.outdir, 'outerdata_train_6var.npy'), outerdata_train)
np.save(os.path.join(args.outdir, 'outerdata_test_6var.npy'), outerdata_val)
np.save(os.path.join(args.outdir, 'innerdata_train_6var.npy'), innerdata_train)
np.save(os.path.join(args.outdir, 'innerdata_val_6var.npy'), innerdata_val)   
np.save(os.path.join(args.outdir, 'innerdata_test_6var.npy'), innerdata_test)      
np.save(os.path.join(args.outdir, 'innerdata_extrasig_train_6var.npy'), innerdata_3prong_extrasig_train)
np.save(os.path.join(args.outdir, 'innerdata_extrasig_val_6var.npy'), innerdata_3prong_extrasig_val)
np.save(os.path.join(args.outdir, 'innerdata_extrabkg_train_6var.npy'), innerdata_extrabkg1_train)
np.save(os.path.join(args.outdir, 'innerdata_extrabkg_val_6var.npy'), innerdata_extrabkg1_val)
np.save(os.path.join(args.outdir, 'innerdata_extrabkg_test_6var.npy'), innerdata_extrabkg2)
